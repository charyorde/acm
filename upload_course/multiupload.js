		...
		if(this.items.length > 0){
			...
			var uId = "";
 			for(var i = 0; i<this.items.length; i++){
				...
				//alert(uploadfile);
				var sampleIconimg = uploadfile + '/images/image.png';
				var sampleIconunknow = uploadfile + '/images/unknown.png';
				var ajaxloader = uploadfile + '/images/ajax-loader.gif';
				var sampleIcon = "<img src= "+sampleIconimg+" />";
				
				var errorClass = "";
				if(typeof this.items[i] != undefined){
					if(self._validate(this.items[i].type) <= 0) {
						sampleIcon = "<img src="+sampleIconunknow+" />";
						errorClass =" invalid";
					} 
					html += '<div class="dfiles'+errorClass+'" rel="'+uId+'"><h5>'+sampleIcon+this.items[i].name+'</h5><div id="'+uId+'" class="progress" style="display:none;"><img src="'+ajaxloader+'" /></div></div>';
				}
			}
